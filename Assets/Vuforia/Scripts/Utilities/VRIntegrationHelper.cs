    ...
    void LateUpdate()
    {
        ...
        if (IsLeft)
        {
            if (mLeftCameraDataAcquired && mRightCameraDataAcquired)
            {
                ...
                mLeftCamera.SetStereoProjectionMatrix(0, mRightCamera.projectionMatrix);
                mRightCamera.SetStereoProjectionMatrix(/*mLeftCamera.projectionMatrix*/0, mRightCamera.projectionMatrix);
